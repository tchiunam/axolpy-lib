...
def test_statefulset_patch_model():
    """
    Test statefulset patch model.
    """

    patch_replicas = 3

    patch = StatefulSetPatch(replicas=patch_replicas)

    assert patch.replicas == patch_replicas
    assert str(patch) == f"{patch.__class__.__name__}" + \
        f"(replicas: {patch_replicas})"

    s = StatefulSet(name="test-statefulset",
    ...
    assert s.patch.replicas == patch_replicas
